...
game.import("extension", function (lib, game, ui, get, ai, _status) {
  ...
  const decadeUIPath = (window.decadeUIPath =
    lib.assetURL + "extension/" + decadeUIName + "/");
  const decadeUIResolvePath = (window.decadeUIResolvePath =
    nonameInitialized + "extension/" + decadeUIName + "/");
  return {
    name: "十周年UI",
    content: function (config, pack) {
      ...
      switch (lib.config.layout) {
        ...
        default:
          alert(
            "十周年UI提醒您，请使用<默认>、<手杀>、<新版>布局以获得良好体验（在选项-外观-布局中调整）。"
          );
          break;
      }
      ...
      window.dui = window.decadeUI = {
        init: function () {
          ...
          var svg = document.body.appendChild(
            document.createElementNS(SVG_NS, "svg")
          );
          var defs = svg.appendChild(document.createElementNS(SVG_NS, "defs"));
          var solo = defs.appendChild(
            document.createElementNS(SVG_NS, "clipPath")
          );
          var duol = defs.appendChild(
            document.createElementNS(SVG_NS, "clipPath")
          );
          var duor = defs.appendChild(
            document.createElementNS(SVG_NS, "clipPath")
          );
          var dskin = defs.appendChild(
            document.createElementNS(SVG_NS, "clipPath")
          );
          ...
          var soloPath = solo.appendChild(
            document.createElementNS(SVG_NS, "path")
          );
          var duoLPath = duol.appendChild(
            document.createElementNS(SVG_NS, "path")
          );
          var duoRPath = duor.appendChild(
            document.createElementNS(SVG_NS, "path")
          );
          var dskinPath = dskin.appendChild(
            document.createElementNS(SVG_NS, "path")
          );
          soloPath.setAttribute(
            "d",
            "M0 0 H1 Q1 0.05 0.9 0.06 Q1 0.06 1 0.11 V1 H0 V0.11 Q0 0.06 0.1 0.06 Q0 0.05 0 0 Z"
          );
          duoLPath.setAttribute(
            "d",
            "M1 0 H0 Q0 0.06 0.15 0.06 Q0 0.06 0 0.11 V1 H1 Z"
          );
          duoRPath.setAttribute(
            "d",
            "M0 0 H1 Q1 0.06 0.85 0.06 Q1 0.06 1 0.11 V1 H0 Z"
          );
          dskinPath.setAttribute(
            "d",
          ...
          */
            ...
            element: {
              ...
              card: {
                ...
                moveTo: function (player) {
                  ...
                  this.fixed = true;
                  var x = Math.round(
                    (player.cacheWidth - arena.cardWidth) / 2 + player.cacheLeft
                  );
                  var y = Math.round(
              ...
              control: {
                ...
                open: function () {
                  ui.control.insertBefore(
                    this,
                    _status.createControl || ui.confirm
                  );
              ...
              player: {
                mark: function (item, info, skill) {
                  ...
                  if (get.itemtype(item) == "cards") {
                    ...
                    for (var i = 0; i < item.length; i++)
                      marks.push(this.mark(item[i], info));
                    return marks;
                  }
                  var mark;
                  if (get.itemtype(item) == "card") {
                    ...
                    if (item.classList.contains("fullborder")) {
                      ...
                      if (!mark.node.mark)
                        mark.node.mark =
                          mark.querySelector(".mark-text") ||
                          decadeUI.element.create("mark-text", mark);
                      mark.node.mark.innerHTML =
                        lib.translate[name.name + "_bg"] ||
                ...
                markSkill: function (name, info, card, nobroadcast) {
                  ...
                  return base.lib.element.player.markSkill.apply(
                    this,
                    arguments
                  );
                },
                unmarkSkill: function (name, info, card, nobroadcast) {
                  ...
                  return base.lib.element.player.unmarkSkill.apply(
                    this,
                    arguments
                  );
                },
                ...
                markSkillCharacter: function (id, target, name, content) {
                  ...
                  game.broadcastAll(
                    function (player, target, name, content, id) {
                      ...
                      player.marks[id].text.style.setProperty(
                        "font-size",
                        "0px",
                        "important"
                      );
                ...
                say: function (str) {
                  ...
                  if (
                    lib.config.background_speak &&
                    lib.quickVoice.indexOf(str) != -1
                  ) {
                    game.playAudio(
                      "voice",
                ...
                updateMark: function (name, storage) {
                  if (!this.marks[name]) {
                    if (
                      lib.skill[name] &&
                      lib.skill[name].intro &&
                      (this.storage[name] || lib.skill[name].intro.markcount)
                    ) {
                      this.markSkill(name);
                  ...
                  if (
                    lib.skill[name] &&
                    lib.skill[name].intro &&
                    !lib.skill[name].intro.nocount &&
                    (this.storage[name] || lib.skill[name].intro.markcount)
                  ) {
                    var num = 0;
                    if (typeof lib.skill[name].intro.markcount == "function") {
                      num = lib.skill[name].intro.markcount(
                        this.storage[name],
                        this
                      );
                      /*-----------------分割线-----------------*/
                    } else if (lib.skill[name].intro.markcount == "expansion") {
                      num = this.countCards("x", (card) =>
                        card.hasGaintag(name)
                      );
                    } else if (
                      typeof this.storage[name + "_markcount"] == "number"
                    ) {
                    ...
                    if (num) {
                      if (!mark.markcount)
                        mark.markcount = decadeUI.element.create(
                          "mark-count",
                          mark
                        );
                ...
                $dieAfter: function () {
                  ...
                  if (!decadeUI.config.playerDieEffect) {
                    if (base.lib.element.player.$dieAfter)
                      base.lib.element.player.$dieAfter.apply(this, arguments);
                    return;
                  }

                  if (!this.node.dieidentity)
                    this.node.dieidentity = ui.create.div(
                      "died-identity",
                      this
                    );
                  ...
                  var url =
                    decadeUIPath +
                    "image/decoration" +
                    (decadeUI.config.newDecadeStyle == "on" ? "" : "s") +
                    "/dead" +
                    (decadeUI.config.newDecadeStyle == "on" ? "" : "2") +
                    "_" +
                    identity +
                    ".png";
                  image.onerror = function () {
                    that.node.dieidentity.innerHTML =
                      decadeUI.getPlayerIdentity(that, that.identity, true) +
                      "<br>阵亡";
                  };

                  // 随机离开效果
                  if (
                    (that._trueMe || that) != game.me &&
                    that != game.me &&
                    Math.random() < 0.5
                  ) {
                    ...
                    if (lib.config.extension_十周年UI_newDecadeStyle == "off") {
                      that.node.dieidentity.innerHTML =
                        '<div style="width:21px; height:81px; left:22px; top:-12px; position:absolute; background-image: url(' +
                        lib.assetURL +
                        'extension/十周年UI/assets/image/likai_2.png);background-size: 100% 100%;"></div>';
                    }

                    if (
                      lib.config.extension_十周年UI_newDecadeStyle == "othersOn"
                    ) {
                      that.node.dieidentity.innerHTML =
                  ...
                  that.node.dieidentity.style.backgroundImage =
                    'url("' + url + '")';
                  image.src = url;
                  setTimeout(function () {
                ...
                $skill: function (name, type, color, avatar) {
                  if (
                    !decadeUI.config.gameAnimationEffect ||
                    !decadeUI.animation.gl
                  )
                    return base.lib.element.player.$skill.apply(
                      this,
                      arguments
                    );
                  var _this = this;
                ...
                $syncExpand: function (map) {
                  ...
                  const goon =
                    lib.skill.expandedSlots.intro.markcount(null, game.me) > 0;
                  ...
                  for (var repetition = 0; repetition < 5; repetition++) {
                    if (storage && storage["equip" + (repetition + 1)]) {
                      for (
                        var adde = 0;
                        adde < storage["equip" + (repetition + 1)];
                        adde++
                      ) {
                        var addediv = decadeUI.element.create(
                          null,
                          equipSolts.back
                        );
                ...
                $init: function (character, character2) {
                  ...
                  this.doubleAvatar =
                    (character2 && lib.character[character2]) != undefined;
                  ...
                  var showDynamic =
                    (this.dynamic || CUR_DYNAMIC < MAX_DYNAMIC) &&
                    duicfg.dynamicSkin;
                  if (showDynamic && _status.mode != null) {
                    var skins;
                    var dskins = decadeUI.dynamicSkin;
                    var avatars = this.doubleAvatar
                    ...
                    for (var i = 0; i < avatars.length; i++) {
                      ...
                      if (keys.length == 0) {
                        console.error(
                          "player.init: " + avatars[i] + " 没有设置动皮参数"
                        );
                        continue;
                      }
                      ...
                      this.$dynamicWrap.style.backgroundImage =
                        'url("' +
                        decadeUIPath +
                        "assets/dynamic/" +
                        skin.background +
                ...
                setSeatNum: function () {
                  ...
                  game.broadcastAll(function (player) {
                    if (!player.node.seat)
                      player.node.seat = decadeUI.element.create(
                        "seat",
                        player
                      );
                    player.node.seat.innerHTML = get.cnNumber(
                      player.seat,
                      true
                    );
                  }, this);
                ...
                $update: function () {
                  ...
                  if (this.hujia > 0) {
                    ...
                    hujiat.innerText =
                      this.hujia == Infinity ? "∞" : this.hujia;
                ...
                $addVirtualJudge(VCard, cards) {
                  const player = this;
                  const isViewAsCard =
                    cards.length !== 1 || cards[0].name !== VCard.name;
                  cards.forEach((card) => {
                    card.fix();
                    ...
                    const bgMark =
                      lib.translate[VCard.name + "_bg"] ||
                      get.translation(VCard.name)[0];
                    if (isViewAsCard) {
                ...
                useCard: function () {
                  var event = base.lib.element.player.useCard.apply(
                    this,
                    arguments
                  );
                ...
                lose: function () {
                  var next = base.lib.element.player.lose.apply(
                    this,
                    arguments
                  );
                ...
                $draw: function (num, init, config) {
                  if (game.chess)
                    return base.lib.element.player.$draw.call(
                      this,
                      num,
                      init,
                      config
                  ...
                  if (_status.event && _status.event.name) {
                    if (
                      (function (event) {
                        return (
                          event.name != "gain" && !event.name.includes("raw")
                        );
                      })(_status.event)
                    )
                      isDrawCard = true;
                ...
                $give: function (cards, target, log, record) {
                  ...
                  if (record !== false) {
                    ...
                    if (itemtype == "number") {
                      cards2 = cards.length;
                      game.addVideo("give", this, [
                        cards2,
                        target.dataset.position,
                      ]);
                    } else {
                      game.addVideo("giveCard", this, [
                        get.cardsInfo(cards2),
                        target.dataset.position,
                      ]);
                    }
                ...
                $handleEquipChange: function () {
                  base.lib.element.player.$handleEquipChange.apply(
                    this,
                    arguments
                  );
                  const player = this;
                  ...
                  if (delta > 0) {
                    while (delta > 0) {
                      delta--;
                      const ediv = decadeUI.element.create(
                        null,
                        ui.equipSolts.back
                      );
                      ediv.dataset.type = 5;
                ...
                removeVirtualEquip: function () {
                  base.lib.element.player.removeVirtualEquip.apply(
                    this,
                    arguments
                  );
                ...
                $throw: function (cards, time, record, nosource) {
                  ...
                  if (typeof cards == "number") {
                    ...
                    if (itemtype == "cards") {
                      ...
                      if (evt && evt.card && evt.cards === cards) {
                        ...
                        if (evt.card.suit == "none")
                          card.node.suitnum.style.display = "none";
                  ...
                  for (var i = 0; i < cards.length; i++) {
                    ...
                    if (card) {
                      ...
                      if (
                        duiMod &&
                        (card.throwWith == "h" || card.throwWith == "s")
                      ) {
                ...
                $throwordered2: function (card, nosource) {
                  ...
                  if (card.throwordered == undefined) {
                    ...
                    if (nosource) {
                      x =
                        (bounds.width - bounds.cardWidth) / 2 -
                        bounds.width * 0.08;
                      y = (bounds.height - bounds.cardHeight) / 2;
                    } else {
                      x =
                        (this.cacheWidth - bounds.cardWidth) / 2 +
                        this.cacheLeft;
                      y =
                        (this.cacheHeight - bounds.cardHeight) / 2 +
                        this.cacheTop;
                    }
              ...
              content: {
                  ...
                  var gainTo = function (cards, nodelay) {
                    ...
                    if (player == game.me) {
                      ...
                      game.addVideo("gain12", player, [
                        get.cardsInfo(fragment.childNodes),
                        gaintag,
                      ]);
                    }
                ...
                judge: function () {
                  "step 0";
                  var judgestr =
                    get.translation(player) + "的" + event.judgestr + "判定";
                ...
                lose: function () {
                  ...
                  if (
                    ...
                    if (event.blameEvent && event.animate == undefined)
                      event.animate = evt.animate;
                  }
                  ("step 1");
                  event.gaintag_map = {};
                  ...
                  for (var i = 0; i < cards.length; i++) {
                    ...
                    if (!hej.includes(card)) {
                      ...
                      if (pileNode.classList.contains("equips")) {
                        ...
                        loseCards.forEach((cardi) => {
                          ...
                          event.vcard_map.set(
                            cardi,
                            card.card || get.autoViewAs(card, void 0, false)
                          );
                        });
                        ...
                        const VJudge = player
                          .getVCards("j")
                          .find((card) => card.cards?.includes(card));
                        ...
                        if (card.classList.contains("glows")) {
                          ...
                          event.vcard_map.set(
                            card,
                            get.autoViewAs(card, void 0, player)
                          );
                        }
                      } else {
                    ...
                    for (var j = 0; j < cardx.length; j++) {
                      if (cardx[j].gaintag && cardx[j].gaintag.length) {
                        event.gaintag_map[cardx[j].cardid] =
                          cardx[j].gaintag.slice(0);
                  ...
                  if (hs.length && !event.visible) {
                    player
                      .getCards("h")
                      .forEach((hcard) => hcard.clearKnowers());
                  }
                  ("step 2");
                  if (num < cards.length) {
                    if (
                      event.es.includes(cards[num]) ||
                      cards[num].cards?.some((i) => event.es.includes(i))
                    ) {
                  ...
                  if (event.toRenku) {
                    ...
                    if (_status.renku.length > 6) {
                      var cards = _status.renku.splice(
                        0,
                        _status.renku.length - 6
                      );
                      game.log(cards, "从仁库进入了弃牌堆");
                      game
                        .cardsDiscard(cards)
                        .set("outRange", true).fromRenku = true;
                    }
                    game.updateRenku();
                  }
                  ("step 5");
                ...
                turnOver: function () {
                  ...
                  game.addVideo(
                    "turnOver",
                    player,
                    player.classList.contains("turnedover")
                  );
          ...
          ride.ui = {
            ...
            updatej: function (player) {
              ...
              for (var i = 0; i < judges.length; i++) {
                ...
                if (_status.connectMode) {
                  const bgMark =
                    lib.translate[judges[i].name + "_bg"] ||
                    get.translation(judges[i].name)[0];
            ...
            update: function () {
              ...
              if (
                ui.dialog == undefined ||
                ui.dialog.classList.contains("noupdate")
              )
                return;
              ...
              if (
                ...
                ui.dialog.style.height =
                  Math.min(height, ui.dialog.content.offsetHeight) + "px";
              }
            ...
            create: {
              ...
              dialog: function () {
                ...
                dialog.content = decadeUI.element.create(
                  "content",
                ...
                if (!notouchscroll) {
                  dialog.contentContainer.ontouchstart =
                    ui.click.dialogtouchStart;
                  dialog.contentContainer.ontouchmove = ui.click.touchScroll;
                  dialog.contentContainer.style.WebkitOverflowScrolling =
                    "touch";
              ...
              identityCard: function (identity, position, info, noclick) {
                ...
                const fileName =
                  "extension/十周年UI/image/identityCard/mougong_" +
                  identity +
                  ".jpg";
              ...
              buttonPresets: {
                character: function (item, type, position, noclick, node) {
                    ...
                    if (
                      ...
                      const check =
                        ...
                        ((get.mode() == "guozhan" ||
                          ...
                          (_status.connectMode ||
                            (_status.connectMode
                              ? lib.configOL.double_hp
                              : get.config("double_hp")) == "pingjun"));
                    ...
                    if (infoitem[1]) {
                      if (doubleCamp) {
                        ...
                        if (doubleCamp.length > 4)
                          if (new Set([5, 6, 9]).has(doubleCamp.length))
                  ...
                  node.refresh(
                    node,
                    item,
                    position ? position.intersection : undefined
                  );
              ...
              card: function (e) {
                ...
                if (
                  ...
                  (this.parentNode.classList.contains("judges") ||
                    this.parentNode.classList.contains("dui-marks"))
                ) {
                ...
                if (this.classList.contains("selected")) {
                  ...
                  if (
                    ...
                    !get.info(skill).ignoreMod
                  ) {
                    const cardskb =
                      typeof get.info(skill).viewAs == "function"
                        ? get.info(skill).viewAs([this], _status.event.player)
                        : get.info(skill).viewAs;
                    ...
                    if (
                      rname != vname ||
                      !get.is.sameNature(rnature, vnature, true)
                    ) {
                      if (this._tempName) {
                        this._tempName.delete();
                        delete this._tempName;
                      ...
                      if (lib.config.extension_十周年UI_showTemp) {
                        if (!this._tempName)
                          this._tempName = ui.create.div(".temp-name", this);
                        let tempname = "",
                        ...
                        const nodeviewas = ui.create.cardTempName(
                          cardskb,
                          this
                        );
                  ...
                  if (range) {
                    if (typeof range.attack === "number") {
                      player.createRangeShadow(
                        Math.min(
                          8,
                          player.getAttackRange(true) + range.attack - 1
                        )
                      );
            ...
            logv: function (player, card, targets, event, forced, logvid) {
              ...
              if (typeof card == "string") {
                ...
                if (!Array.isArray(node.cards) || !node.cards.length) {
                  node.cards = [
                    ui.create.card(node, "noclick", true).init(info),
                  ];
                }
                if (card.name == "wuxie") {
                  if (
          ...
          lib.hooks["checkTarget"].push(function decadeUI_selectable(
            target,
            event
          ) {
            const list = ["selected", "selectable"];
            target.classList[
              list.some((select) => target.classList.contains(select))
                ? "remove"
                : "add"
            ]("un-selectable");
          });
          //对十周年UI和本体的视为卡牌样式的同时适配
          const updateTempname = lib.hooks["checkCard"].indexOf(
            lib.hooks["checkCard"].find(
              (i) => i.name && i.name == "updateTempname"
            )
          );
          lib.hooks["checkCard"][updateTempname] = function updateTempname(
            card,
            event
          ) {
            ...
            if (
              card.name !== cardname ||
              !get.is.sameNature(card.nature, cardnature, true)
            ) {
              if (lib.config.extension_十周年UI_showTemp) {
                if (!card._tempName)
                ...
                const node = goon
                  ? ui.create.cardTempName(cardskb, card)
                  : ui.create.cardTempName(card);
                if (lib.config.cardtempname !== "default")
              ...
              if (_status.event.type == "phase") {
                const innerHTML =
                  lib.config["extension_十周年UI_newDecadeStyle"] == "on"
                    ? "回合结束"
                    : "结束出牌";
                ui.confirm.lastChild.innerHTML = _status.event.skill
                  ? "取消"
                  : innerHTML;
              }
            }
          ...
          //game.uncheck修改
          //对十周年UI和本体的视为卡牌样式的同时适配
          const removeTempname = lib.hooks["uncheckCard"].indexOf(
            lib.hooks["uncheckCard"].find(
              (i) => i.name && i.name == "removeTempname"
            )
          );
          lib.hooks["uncheckCard"][removeTempname] = function removeTempname(
            card,
            event
          ) {
            if (card._tempName) {
          ...
          lib.hooks["uncheckTarget"].push(function decadeUI_unselectable(
            target,
            event
          ) {
          ...
          const updateDialog = lib.hooks["checkOverflow"].indexOf(
            lib.hooks["checkOverflow"].find(
              (i) => i.name && i.name == "updateDialog"
            )
          );
          lib.hooks["checkOverflow"][updateDialog] = function updateDialog(
            itemOption,
            itemContainer,
            addedItems,
            game
            ...
            const W =
              addedItems[0].getBoundingClientRect().width / game.documentZoom;
          ...
          game.linexy = function (path) {
            if (!decadeUI.config.playerLineEffect)
              return gameLinexyFunction.apply(this, arguments);
            decadeUI.effect.line(path);
          };

          ui.click.intro = function (e) {
            if (this.classList.contains("infohidden") || _status.dragged)
              return;
            _status.clicked = true;
          ...
          ui.click.identity = function (e) {
            if (
              _status.dragged ||
              !game.getIdentityList ||
              _status.video ||
              this.parentNode.forceShown
            )
              return;
            _status.clicked = true;
            ...
            if (lib.config.mark_identity_style == "click") {
              ...
              if (!dui.$identityMarkBox) {
                dui.$identityMarkBox =
                  decadeUI.element.create("identity-mark-box");
          ...
          ui.click.volumn = function () {
            ...
            var backVolume = decadeUI.component.slider(
              0,
              8,
              parseInt(lib.config.volumn_background)
            );
            var gameVolume = decadeUI.component.slider(
              0,
              8,
              parseInt(lib.config.volumn_audio)
            );
          ...
          ui.create.me = function (hasme) {
            ...
            ui.handcards1Container = decadeUI.element.create(
              "hand-cards",
              ui.me
            );
            ui.handcards1Container.onmousewheel =
              decadeUI.handler.handMousewheel;

            ui.handcards2Container = ui.create.div("#handcards2");
            ui.arena.classList.remove("nome");

            var equipSolts = (ui.equipSolts =
              decadeUI.element.create("equips-wrap"));
            equipSolts.back = decadeUI.element.create(
              "equips-back",
              equipSolts
            );
            /*
          ...
          ui.create.player = function (position, noclick) {
            ...
            var playerExtend = {
              node: {
                avatar: ui.create
                  .div(".primary-avatar", player, ui.click.avatar)
                  .hide(),
                avatar2: ui.create
                  .div(".deputy-avatar", player, ui.click.avatar2)
                  .hide(),
            ...
            chainImg.onerror = function () {
              var node = decadeUI.element.create(
                "chain-back",
                player.node.chain
              );
              for (var i = 0; i < 40; i++)
                decadeUI.element.create("cardbg", node).style.transform =
                  "translateX(" + (i * 5 - 5) + "px)";
            ...
            playerExtend.node.handcards1._childNodesWatcher =
              new ChildNodesWatcher(playerExtend.node.handcards1);
            playerExtend.node.handcards2._childNodesWatcher =
              new ChildNodesWatcher(playerExtend.node.handcards2);
                ...
                set: function (value) {
                  ...
                  switch (value) {
                    ...
                    case "主":
                      ...
                      if (
                        get.mode() == "versus" &&
                        get.translation(player.side + "Color") == "wei"
                      ) {
                        filename += "_blue";
                  ...
                  if (decadeUI.config.campIdentityImageMode) {
                    ...
                    if (lib.config.extension_十周年UI_newDecadeStyle == "on") {
                      image.src =
                        decadeUIPath +
                        "image/decoration/identity_" +
                        filename +
                        ".png";
                    } else {
                      image.src =
                        decadeUIPath +
                        "image/decorations/identity2_" +
                        filename +
                        ".png";
                    }
                    this.parentNode.style.backgroundImage =
            ...
            if (!noclick) {
              ...
              player.node.identity.addEventListener(
                lib.config.touchscreen ? "touchend" : "click",
                ui.click.identity
              );
              if (lib.config.touchscreen) {
            ...
            var campWrapExtend = {
              node: {
                ...
                avatarDefaultName: decadeUI.element.create(
                  "avatar-name-default",
                  campWrap
                ),
              },
            ...
            var node = {
              mask: player.insertBefore(
                decadeUI.element.create("mask"),
                player.node.identity
              ),
          ...
          ui.create.card = function (position, info, noclick) {
            ...
            card.$suitnum.$num = decadeUI.element.create(
              null,
              card.$suitnum,
              "span"
            );
            ...
            card.$suitnum.$br = decadeUI.element.create(
              null,
              card.$suitnum,
              "br"
            );
            card.$suitnum.$suit = decadeUI.element.create(
              "suit",
              card.$suitnum,
              "span"
            );
            card.$suitnum.$suit.style.fontFamily =
              '"STHeiti","SimHei","Microsoft JhengHei","Microsoft YaHei","WenQuanYi Micro Hei",Helvetica,Arial,sans-serif';
            card.$equip.$suitnum = decadeUI.element.create(
              null,
              card.$equip,
              "span"
            );
            card.$equip.$name = decadeUI.element.create(
              null,
              card.$equip,
              "span"
            );
          ...
          lib.element.content.chooseToGuanxing = function () {
                ...
                function (list) {
                  ...
                  switch (name) {
                    ...
                    case "phaseZhunbei":
                      ...
                      cards.sort((a, b) => {
                        return (
                          (get.value(b, target) - get.value(a, target)) * att
                        );
                      });
                      while (needs--) {
                        top.unshift(cards.shift());
            ...
            player.popup(
              get.cnNumber(event.num1) + "上" + get.cnNumber(event.num2) + "下"
            );
            game.logv(
              player,
              "将" +
          ...
          lib.element.player.addSkill = function (skill) {
            ...
            if (!Array.isArray(skill)) {
              const skills = ["name", "name1", "name2"].reduce((list, name) => {
                if (
                  this[name] &&
                  (name != "name1" || this.name != this.name1)
                ) {
          ...
          lib.element.player.removeSkill = function (skill) {
            ...
            if (!Array.isArray(skill)) {
              if (
                this.node.gainSkill.skills &&
                this.node.gainSkill.skills.includes(skill)
              ) {
          ...
          Object.defineProperties(lib.element.player, {
            group: {
              ...
              set: function (group) {
                if (!group) return;
                this._group = group;
                this.node.campWrap.dataset.camp =
                  get.character(this.name)?.groupBorder || group;
                if (!decadeUI.config.campIdentityImageMode) {
                  if (!this._finalGroup)
                ...
                var url =
                  ...
                  (decadeUI.config.newDecadeStyle == "off"
                    ? "image/decorations/name2_"
                    : "image/decoration/name_") +
                  group +
                  ".png";
          ...
          lib.element.player.setModeState = function (info) {
            if (info && info.seat) {
              if (!this.node.seat)
                this.node.seat = decadeUI.element.create("seat", this);
              this.node.seat.innerHTML = get.cnNumber(info.seat, true);
            }

            if (base.lib.element.player.setModeState) {
              return base.lib.element.player.setModeState.apply(
                this,
                arguments
              );
            } else {
          ...
          lib.element.player.$damagepop = function (
            num,
            nature,
            font,
            nobroadcast
                ...
                if (popupNum < 0) {
                  switch (node.nature) {
                    case "thunder":
                      if (popupNum <= -2) {
                        decadeUI.animation.playSpine(
                          { name: "effect_shoujidonghua", action: "play6" },
                          { scale: 0.8, parent: player }
                        );
                      } else {
                        decadeUI.animation.playSpine(
                          { name: "effect_shoujidonghua", action: "play5" },
                          { scale: 0.8, parent: player }
                        );
                      }
                      break;
                    case "fire":
                      if (popupNum <= -2) {
                        decadeUI.animation.playSpine(
                          { name: "effect_shoujidonghua", action: "play4" },
                          { scale: 0.8, parent: player }
                        );
                      } else {
                        decadeUI.animation.playSpine(
                          { name: "effect_shoujidonghua", action: "play3" },
                          { scale: 0.8, parent: player }
                        );
                      }
                      break;
                    ...
                    default:
                      if (popupNum <= -2) {
                        decadeUI.animation.playSpine(
                          { name: "effect_shoujidonghua", action: "play2" },
                          { scale: 0.8, parent: player }
                        );
                      } else {
                        decadeUI.animation.playSpine(
                          { name: "effect_shoujidonghua", action: "play1" },
                          { scale: 0.8, parent: player }
                        );
                      }
                      break;
          ...
          lib.element.player.$compareMultiple = function (
            card1,
            targets,
            cards
          ) {
          ...
          lib.element.card.copy = function () {
            ...
            if (
              asset &&
              !asset.loaded &&
              clone.classList.contains("decade-card")
            ) {
              if (asset.loaded == undefined) {
        ...
        dialog: {
          create: function (className, parentNode, tagName) {
            ...
            for (var i in decadeUI.dialog.listens) {
              if (decadeUI.dialog.listens[i])
                element.listens[i] = decadeUI.dialog.listens[i];
            }
          ...
          close: function (delayTime, fadeOut) {
            ...
            if (delayTime) {
              var remove = this;
              delayTime =
                typeof delayTime == "number" ? delayTime : parseInt(delayTime);
              setTimeout(function () {
          ...
          listens: {
            add: function (listenElement, event, func, useCapture) {
              if (!this._dialog || !this._list)
                return console.error("undefined");
              if (
                !(listenElement instanceof HTMLElement) ||
                !event ||
            ...
            remove: function (
              ...
              if (listenElementOptional && eventOptional && funcOptional) {
                var index = list.indexOf(
                  new Array(listenElementOptional, eventOptional, funcOptional)
                );
                if (index != -1) {
                  list[index][0].removeEventListener(
                    list[index][1],
                    list[index][2]
                  );
                  list.splice(index, 1);
                ...
                for (var i = list.length - 1; i >= 0; i--) {
                  if (
                    list[i][0] == listenElementOptional &&
                    list[i][1] == eventOptional
                  ) {
        ...
        animate: {
          check: function () {
            ...
            if (this.canvas == undefined) {
              this.canvas = ui.arena.appendChild(
          ...
          update: function () {
            ...
            var delta =
              frameTime -
              (this.frameTime == undefined ? frameTime : this.frameTime);
        ...
        sheet: {
            ...
            if (!this.sheetList) {
              ...
              for (var i = 0; i < document.styleSheets.length; i++) {
                if (
                  ...
                  document.styleSheets[i].href.indexOf(
                    "extension/" + encodeURI(decadeUIName)
                  ) != -1
                ) {
          ...
          getStyle: function (selector, cssName) {
            ...
            if (typeof selector != "string" || !selector)
              throw 'parameter "selector" error';
        ...
        layout: {
          ...
          updateHand: function () {
            ...
            if (totalW > limitW) {
              xMargin =
                csw -
                Math.abs(limitW - csw * cards.length) / (cards.length - 1);
              if (lib.config.fold_card) {
                var foldCardMinWidth =
                  lib.config.extension_十周年UI_foldCardMinWidth;
                var min = cs;
                if (foldCardMinWidth == "cardWidth") {
                  min *= cw;
                } else {
                  min *=
                    foldCardMinWidth && foldCardMinWidth.length
                      ? parseInt(foldCardMinWidth)
                      : 81;
                }
                if (xMargin < min) {
                  expand = true;
            ...
            if (expand) {
              ...
              handNode.style.width =
                Math.round(cards.length * xMargin + (csw - xMargin)) + "px";
            } else {
              /*-----------------分割线-----------------*/
              // 手牌滑动，咸鱼大佬提供代码
          ...
          updateDiscard: function () {
            ...
            for (var i = ui.thrown.length - 1; i >= 0; i--) {
              if (
                ui.thrown[i].classList.contains("drawingcard") ||
          ...
          resize: function () {
            ...
            if (ui.me) {
              var height =
                Math.round(
                  decadeUI.getHandCardSize().height * decadeUI.zooms.card + 30.4
                ) + "px";
        ...
        delay: function (milliseconds) {
          if (typeof milliseconds != "number")
            throw "milliseconds is not number";
        ...
        layoutHandDraws: function (cards) {
          ...
          if (source && source != game.me) {
            ...
            y = Math.round(
              source.cacheTop - bounds.y - 30 + (source.cacheHeight - ch) / 2
            );
            for (var i = 0; i < cards.length; i++) {
              x = Math.round(xStart + i * xMargin);
              card = cards[i];
        ...
        layoutDrawCards: function (cards, player, center) {
          ...
          var xMargin = 27;
          var xStart =
            (center ? (pw - playerW) / 2 : playerX) - csw / 2 - (cw - csw) / 2;
          var totalW = xMargin * cards.length + (csw - xMargin);
          var limitW = playerW + csw;

          if (totalW > limitW) {
            xMargin =
              csw - Math.abs(limitW - csw * cards.length) / (cards.length - 1);
          } else {
            xStart += (limitW - totalW) / 2;
          }
        ...
        tryAddPlayerCardUseTag: function (card, player, event) {
          ...
          switch (event.name.toLowerCase()) {
            ...
            case "respond":
              ...
              if (
                ...
                (card.name != cardname ||
                  !get.is.sameNature(cardnature, card.nature, true))
              ) {
                if (lib.config.extension_十周年UI_showTemp) {
                  if (!card._tempName)
                    card._tempName = ui.create.div(".temp-name", card);
            ...
            case "useskill":
              tagText =
                "发动" + get.skillTranslation(event.skill, event.player);
              break;
            case "die":
              tagText = "弃置";
              card.classList.add("invalided");
            ...
            case "judge":
              ...
              event.addMessageHook("judgeResult", function () {
                ...
                if (tagNode == null)
                  tagNode = card.appendChild(dui.element.create("used-info"));
                if (
                  event.result.suit != get.suit(card) ||
                  event.result.number != get.number(card)
                ) {
                  dui.cardTempSuitNum(
                    card,
                    event.result.suit,
                ...
                if (getEffect) {
                  ...
                  judgeValue = decadeUI.get.judgeEffect(
                    event.judgestr,
                    event.result.judge
                  );
                }
          ...
          tagNode.textContent =
            (noname ? "" : get.translation(event.player)) + tagText;
        },
        ...
        getHandCardSize: function (canUseDefault) {
          ...
          if (style == null)
            style = decadeUI.sheet.getStyle(".hand-cards > .handcards > .card");
          if (style == null)
        ...
        getMapElementPos: function (elementFrom, elementTo) {
          if (
            !(elementFrom instanceof HTMLElement) ||
            !(elementTo instanceof HTMLElement)
          )
            return console.error("arguments");
        ...
        getPlayerIdentity: function (player, identity, chinese, isMark) {
          if (
            !(player instanceof HTMLElement && get.itemtype(player) == "player")
          )
            throw "player";
          ...
          if (!chinese) {
            switch (mode) {
              case "identity":
                if (
                  ...
                  identity = (
                    player.special_identity ? player.special_identity : identity
                  ).replace(/identity_/, "");
                }
              ...
              case "guozhan":
                if (identity == "unknown") {
                  identity = player.wontYe()
                    ? lib.character[player.name1][1]
                    : "ye";
                }
            ...
            switch (mode) {
                ...
                if (identity.indexOf("cai") < 0) {
                  if (isMark) {
                    if (player.special_identity)
                      identity = player.special_identity + "_bg";
                  } else {
                    identity = player.special_identity
              ...
              case "guozhan":
                if (identity == "unknown") {
                  identity = player.wontYe()
                    ? player.trueIdentity || lib.character[player.name1][1]
                    : "ye";
                }
        ...
        create: {
          ...
          compareDialog: function (player, target) {
            ...
            var extend = {
              ...
              $player: decadeUI.element.create(
                "player-character player1",
                dialog.$content
              ),
              $target: decadeUI.element.create(
                "player-character player2",
                dialog.$content
              ),
              $playerCard: decadeUI.element.create(
                "player-card",
                dialog.$content
              ),
              $targetCard: decadeUI.element.create(
                "target-card",
                dialog.$content
              ),
              ...
              Object.defineProperties(dialog, {
                player: {
                  ...
                  set: function (value) {
                    ...
                    if (value)
                      this.$playerCard.dataset.text =
                        get.translation(value) + "发起";
                  },
                },
                target: {
                  configurable: true,
                  ...
                  set: function (value) {
                    ...
                    if (value)
                      this.$targetCard.dataset.text = get.translation(value);
                  },
                },
                playerCard: {
                  configurable: true,
        ...
        get: {
          ...
          ease: function (fraction) {
            if (!decadeUI.get._bezier3)
              decadeUI.get._bezier3 = new duilib.CubicBezierEase(
                0.25,
                0.1,
                0.25,
                1
        ...
        set: (function (set) {
          set.activeElement = function (element) {
            ...
            if (
              deactive &&
              deactive != element &&
              typeof deactive.ondeactive == "function"
            ) {
              deactive.ondeactive();
            }
            ...
            if (
              element &&
              element != deactive &&
              typeof element.onactive == "function"
            ) {
              element.onactive();
            }
          };
        ...
        statics: {
          cards: (function (cards) {
            ...
            if (window.fs) {
              fs.readdir(
                __dirname + "/" + decadeUIPath + "image/card/",
                function (err, files) {
                  if (err) return;
              ...
              resolveLocalFileSystemURL(
                decadeUIResolvePath + "image/card/",
                function (entry) {
          ...
          if (!game.loopLocked) {
            ...
            try {
              if (
                ...
                !decadeUI.eventDialog.finished &&
                !decadeUI.eventDialog.finishing
              ) {
      ...
      lib.element.player.trySkillAnimate = function (name, popname, checkShow) {
        ...
        if (
          ...
          lib.config.extension_十周年UI_jindutiao == true
        ) {
          if (!document.querySelector("#jindutiaopl") && that == game.me) {
            ...
            if (
              ...
              //--------技能文本-----//
              tipskilltext.innerHTML = get
                .skillTranslation(name, that)
                .slice(0, 2);
      ...
      ui.create.characterDialog = function () {
        ...
        const control =
          lib.config.extension_十周年UI_mx_decade_characterDialog || "default";
        if (control != "default") {
          ...
          if (control == "extension-OL-system") {
            ...
            input.onkeydown = function (e) {
              ...
              if (e.keyCode == 13) {
                ...
                var choice =
                  div.querySelector("select").options[
                    div.querySelector("select").selectedIndex
                  ].value;
                if (value) {
                    ...
                    if (
                      (function (choice, value, name, skills) {
                        ...
                        if (choice == "name1") return goon(value, name);
                          ...
                          return skills.some((skill) =>
                            goon(value, get.translation(skill + "_info"))
                          );
                ...
                if (dialog.paginationMaxCount.get("character")) {
                  ...
                  if (p) {
                    const array = dialog.buttons.filter(
                      (item) => !item.classList.contains("nodisplay")
                    );
                    p.state.data = array;
                    p.setTotalPageCount(
                      Math.ceil(
      ...
      lib.element.player.$changeZhuanhuanji = function (skill) {
        ...
        var mark = this.node.xSkillMarks.querySelector(
          '[data-id="' + skill + '"]'
        );
          ...
          if (lib.config.extension_十周年UI_newDecadeStyle == "on") {
            ...
            if (mark.dd == true) {
              ...
              if (mark.dk) {
                ...
                mark.setBackgroundImage(
                  "extension/十周年UI/shoushaUI/skill/images/ditu_ying.png"
                );
              }
            }
          }
          ...
          if (event.arrangeSkill) {
            ...
            if (hidden.length) {
              for (var i of event.controls) {
                if (
                  _status.prehidden_skills.includes(i) &&
                  hidden.includes(i)
                ) {
                  event.result = {
                    bool: true,
          ...
          if (event.sortcard) {
            ...
            if (event.seperate || lib.config.seperate_control) {
              ...
              if ((event.direct && controls.length == 1) || event.forceDirect) {
                ...
                event.controlbars = [];
                for (var i = 0; i < event.controls.length; i++) {
                  event.controlbars.push(
                    ui.create.control([event.controls[i]])
                  );
                }
              }
            } else {
            ...
            if (event.dialog) {
              ...
              for (var i = 0; i < list.length; i++) {
                ...
                //--------背水-----//
                if (
                  list[i].childNodes[0].innerText.indexOf("背水") != -1 &&
                  lib.config.extension_十周年UI_newDecadeStyle != "on"
                ) {
              ...
              if (event.prompt2) {
                event.dialog.addText(
                  event.prompt2,
                  event.prompt2.length <= 20 || event.centerprompt2
                );
              }
            }
          }
      ...
      if (get.mode() != "connect") {
        ...
        lib.skill._jindutiaoO = {
          ...
          filter: function (event, player) {
            if (
              document.querySelector("#jindutiaoAI") &&
              lib.config.extension_十周年UI_jindutiaoaiUpdata == false
            )
              return false;
      ...
      lib.skill._chupaiJ = {
        ...
        filter: function (event, player) {
          ...
          return (
            h.length > 0 && player != game.me && _status.currentPhase != player
          );
        },
        content: function () {
      ...
      lib.skill._chupaiNX = {
        ...
        filter: function (event, player) {
          ...
          return (
            e.length > 0 && player != game.me && _status.currentPhase != player
          );
        },
        content: function () {
      ...
      if (config.GTBB) {
        ...
        var gddf = function () {
          ...
          //更改对应播报颜色
          var gold = [
            '<font color="#56e4fa">' + pifu + "</font>",
            '<font color="#f3c20f">' + wujiang + "</font>",
          ].randomGet();
          var d = [
            ",大家快恭喜TA吧！",
            ",大家快恭喜TA吧。无名杀是一款非盈利游戏(づ ●─● )づ",
            ",祝你新的一年天天开心，万事如意",
          ].randomGet();
          /*定义部分属性--默认手杀*/
        ...
        if (config.GTBBYangshi == "on") {
          ...
          txcsanm.div.style["background-image"] =
            "url(" +
            lib.assetURL +
        ...
        var id = setInterval(function () {
          if (!txcsanm.div.parentNode && ui.window) {
            ...
            setInterval(
              gddf,
              parseFloat(lib.config["extension_十周年UI_GTBBTime"])
            );
          }
        }, 5000);
      }
        ...
        lib.skill._jd_ddxyA = {
          ...
          content: function () {
            ...
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                get.mode() == "taixuhuanjing" ||
                lib.config["extension_EngEX_SSServant"]
              ) {
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/ddxy.jpg",
                  [10, 58, 7, 6],
                  10
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/ddxy.png",
                [18, 65, 8, 4.4],
                10
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/dfsk.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/dfsk.png",
                [18, 65, 8, 4.4],
                true
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/hhks.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/hhks.png",
                [18, 65, 8, 4.4],
                true
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/pdjd.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/zbjd.png",
                [18, 65, 8, 4.4],
                true
              );
            }
        ...
        lib.skill._jd_pdjdb = {
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/pdjd.png",
                [18, 65, 8, 4.4],
                true
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/mpjd.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/mpjd.png",
                [18, 65, 8, 4.4],
                true
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/cpjd.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/cpjd.png",
                [18, 65, 8, 4.4],
                true
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/qpjd.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/qpjd.png",
                [18, 65, 8, 4.4],
                true
              );
            }
        ...
        lib.skill._jd_jsjdb = {
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/jsjd.png",
                [18, 65, 8, 4.4],
                true
              );
            }
          ...
          content: function () {
            if (lib.config.extension_十周年UI_JDTSYangshi == "1") {
              if (
                ...
                game.as_showImage(
                  "extension/十周年UI/shoushaUI/lbtn/images/JDTS/hhjs.jpg",
                  [10, 58, 7, 6],
                  true
                );
              } else {
              ...
              game.as_showImage(
                "extension/十周年UI/shoushaUI/lbtn/images/JDTS/hhjs.png",
                [18, 65, 8, 4.4],
                true
              );
            }
      ...
      if (get.mode() != "connect" && config.jindutiao == true) {
        ...
        lib.skill._jindutiao = {
          ...
          filter: function (event, player) {
            if (
              document.querySelector("#jindutiaopl") &&
              lib.config.extension_十周年UI_jindutiaoUpdata == false
            )
              return false;
            return player == game.me && _status.currentPhase == player;
          },
        ...
        lib.skill._jindutiaopl = {
          ...
          filter: function (event, player) {
            ...
            if (
              ...
              lib.config["extension_无名补丁_enable"]
            )
              return false;
            return _status.currentPhase != player && player == game.me;
          },
    ...
    precontent: function () {
      ...
      lib.configMenu.appearence.config.layout.visualMenu = (node, link) => {
        ...
        while (list.length) {
          ui.create
            .div(".avatar", ui.create.div(".seat-player.fakeplayer", node))
      ...
      window.decadeModule = (function (decadeModule) {
        ...
        decadeModule.init = function () {
          ...
          if (lib.config.extension_十周年UI_KGMH == "1")
            this.css(layoutPath + "KGMH/" + "kaiguan.css");
          if (lib.config.extension_十周年UI_KGMH == "2")
            this.css(layoutPath + "KGMH/" + "kaiguan_new.css");
        ...
        decadeModule.import = function (module) {
          ...
          if (typeof module != "function")
            return console.error("import failed");
      ...
      Object.defineProperties(_status, {
        connectMode: {
          ...
          set: function (value) {
            ...
            const decadeExtension = lib.extensions.find(
              (value) => value[0] == decadeUIName
            );
            ...
            lib.init.startBefore = function () {
              ...
              if (startBeforeFunction)
                startBeforeFunction.apply(this, arguments);
            };
          },
        },
        _connectMode: {
        ...
        importPlugin: function (data, setText) {
            ...
            if (/\/$/.test(i)) {
              ...
              fileList.push({
                ...
                path:
                  ...
                  app.name +
                  "/" +
                  i.split("/").reverse().slice(1).reverse().join("/"),
                name: i.split("/").pop(),
                target: zip.files[i],
              });
            }
          ...
          var writeFile = function () {
            ...
            if (file) {
              ...
              game.writeFile(
                isNode
                  ? file.target.asNodeBuffer()
                  : file.target.asArrayBuffer(),
                file.path,
                file.name,
                writeFile
              );
        ...
        get: {
          ...
          skillInfo: function (skill, node) {
            ...
            if (node) {
              ...
              if (obj.info.temp || !node.skills.includes(skill))
                obj.temp = true;
              if (obj.info.frequent || obj.info.subfrequent)
                obj.frequent = true;
              if (
                obj.info.clickable &&
                node.isIn() &&
      ...
      if (lib.config.extension_十周年UI_KGMH == "1")
        lib.init.css(layoutPath, "KGMH/kaiguan");
      if (lib.config.extension_十周年UI_KGMH == "2")
        lib.init.css(layoutPath, "KGMH/kaiguan_new");
      if (!(get.mode() == "chess" || get.mode() == "tafang")) {
          ...
          switch (pack) {
            case "card":
              if (bool) {
                lib.init.css(
                  layoutPath + pack,
                  "main1" + (get.is.phoneLayout() ? "" : "_window")
                );
      ...
      game.Jindutiaoplayer = function () {
        ...
        if (lib.config.extension_十周年UI_jindutiaoYangshi == "1") {
          ...
          boxContent.style["boxShadow"] =
            "0px 0px 9px #2e2b27 inset,0px 0px 2.1px #FFFFD5";
          ...
          boxContent.style.bottom =
            parseFloat(lib.config["extension_十周年UI_jindutiaoSet"]) + "%";
        ...
        if (lib.config.extension_十周年UI_jindutiaoYangshi == "2") {
          ...
          imgBg.src =
            lib.assetURL +
            "extension/十周年UI/shoushaUI/lbtn/images/uibutton/jindutiao.png";
          imgBg.style.cssText =
        ...
        if (lib.config.extension_十周年UI_jindutiaoYangshi == "3") {
          ...
          imgBg.src =
            lib.assetURL +
            "extension/十周年UI/shoushaUI/lbtn/images/uibutton/jindutiao2.png";
          imgBg.style.cssText =
        ...
        window.timer = setInterval(function () {
          ...
          if (boxTime.data == 0) {
            ...
            boxContent.remove();
            if (
              lib.config.extension_十周年UI_jindutiaotuoguan == true &&
              _status.auto == false
            ) {
              ui.click.auto();
            }
      ...
      game.showChatWordBackgroundX = function () {
        if (window.chatBg != undefined && window.chatBg.show) {
          ...
          if (window.dialog_lifesay) {
            if (window.dialog_lifesay.show)
              window.dialog_lifesay.style.left =
        ...
        game.open_lifesay = function () {
          ...
          window.dialog_lifesay.style.left =
            "-" + window.dialog_lifesay.style.width; //这里弄一个右移的动画
          setTimeout(function () {
          ...
          window.dialog_lifesay.setBackgroundImage(
            "extension/十周年UI/shoushaUI/sayplay/nobg.png"
          ); //把背景dialog设置为透明
          ...
          for (var i = 0; i < window.lifesayWord.length; i++) {
            window["dialog_lifesayContent_" + i] = ui.create.div(
              "hidden",
              "",
              function () {
                ...
                game.playAudio(
                  "..",
                  "extension",
                  "十周年UI/shoushaUI/sayplay/audio",
                  this.pos + "_" + game.me.sex
                );
              }
            );
            ...
            window["dialog_lifesayContent_" + i].content =
              window.lifesayWord[i];
            window["dialog_lifesayContent_" + i].innerHTML =
              "<font color=white>" + window.lifesayWord[i] + "</font>"; //显示的字体可以自己改
            window.dialog_lifesayBgColor.appendChild(
              window["dialog_lifesayContent_" + i]
            );
        ...
        window.hudongkuang.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/hudong.png"
        );
        ...
        window.meijiu.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/meijiu.png"
        );
        //这里
        ...
        window.xianhua.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/xianhua.png"
        );
        //这里
        ...
        window.tuoxie.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/tuoxie.png"
        );
        //这里
        ...
        //这里
        window.jidan.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/jidan.png"
        );
        ...
        window.cailan.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/cailan.png"
        );
        ...
        window.qicai.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/qicai.png"
        );
        ...
        window.xiaojiu.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/xiaojiu.png"
        );
        ...
        window.xueqiu.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/xueqiu.png"
        );
        ...
        window.xuwu.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/xuwu.png"
        );
        ...
        window.cailanzi.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/cailanzi.png"
        );

        window.chatBg.appendChild(window.cailanzi);

        window.shuliang = ui.create.node("div");
        window.shuliang.innerText = Math.floor(
        ...
        game.open_emoji = function () {
          ...
          for (var i = 0; i < 50; i++) {
            window["dialog_emojiContent_" + i] = ui.create.div(
              "hidden",
              "",
              function () {
                game.me.say(
            ...
            window["dialog_emojiContent_" + i].style.backgroundSize =
              "100% 100%";
            window.dialog_emojiBgColor.appendChild(
              window["dialog_emojiContent_" + i]
            );
        ...
        window.ipt.style["background-image"] =
        ...
        window.ipt.innerHTML =
          '<input type="text" value=' +
          (window.input_value || "请输入文字") +
        ...
        window.input.style.backgroundImage =
          "url('" +
          lib.assetURL +
      ...
      game.showChatWord = function () {
        ...
        window.chatBackground.style.bottom =
          "calc( " + window.chatBg.style.height + " + " + "5px )";
        ...
        window.chatBackgroundPict.setBackgroundImage(
          "extension/十周年UI/shoushaUI/sayplay/saydiv.png"
        );
      ...
      game.as_showText = function (str, pos, time, font, size, color) {
        ...
        div.style.cssText =
          "z-index:-3; pointer-events:none; font-family:" +
          font +
          "; font-size:" +
          size +
          "px; color:" +
      ...
      game.as_showImage = function (url, pos, time) {
        ...
        div.style.cssText =
          "z-index:-1; pointer-events:none; left:" +
          pos[0] +
          "%; top:" +
          pos[1] +
          "%; width:8%; height:" +
    ...
    config: {
      FL0: {
        name:
          "<img style=width:240px src=" +
          lib.assetURL +
          "extension/十周年UI/shoushaUI/line.png>",
        intro: "",
        init: true,
        clear: true,
      },
      ...
      rightLayout: {
        ...
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.rightLayout = lib.config[
              "extension_十周年UI_rightLayout"
            ]
              ? "on"
              : "off";
        },
      ...
      cardAlternateNameVisible: {
        ...
        update: function () {
          if (window.decadeUI)
            ui.window.dataset.cardAlternateNameVisible = lib.config[
              "extension_十周年UI_cardAlternateNameVisible"
            ]
              ? "on"
              : "off";
        },
            ...
            if (cards.length) {
              for (let j = 0; j < cards.length; j++) {
                ...
                if (!goon) {
                  ...
                  cardskb =
                    typeof get.info(skill).viewAs == "function"
                      ? get.info(skill).viewAs([card], game.me)
                      : get.info(skill).viewAs;
                  cardname = get.name(cardskb);
                  cardnature = get.nature(cardskb);
                }
      ...
      borderLevel: {
        ...
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.borderLevel =
              lib.config["extension_十周年UI_borderLevel"];
        },
      },
      gainSkillsVisible: {
        name: "获得技能显示",
        ...
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.gainSkillsVisible =
              lib.config["extension_十周年UI_gainSkillsVisible"];
        },
      },
      foldCardMinWidth: {
        name: "折叠手牌最小宽度",
      ...
      playerMarkStyle: {
        ...
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.playerMarkStyle =
      ...
      newDecadeStyle: {
        ...
        intro:
          '<b><font color="#FF0000">切换武将边框样式和界面布局，初始为十周年样式，根据个人喜好自行切换，选择不同的设置后游戏会自动重启以生效新的设置',
        init: "off",
        item: {
          on: "十周年",
          off: "新手杀",
        ...
        update: function () {
          if (window.decadeUI) {
            ui.arena.dataset.newDecadeStyle =
              lib.config["extension_十周年UI_newDecadeStyle"];
            ui.arena.dataset.decadeLayout =
              lib.config["extension_十周年UI_newDecadeStyle"] == "on"
                ? "on"
                : "off";
          }
      ...
      shadowStyle: {
        ...
        intro:
          '<b><font color="#FF9000">可根据个人喜好切换局内阴影动态特效与人物弹出文字的样式，目前只有新手杀样式可用',
        init: "on",
        item: {
          on: "原样式",
          off: "新样式",
        ...
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.shadowStyle =
              lib.config["extension_十周年UI_shadowStyle"];
        },
      },
      longLevel: {
        ...
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.longLevel =
              lib.config["extension_十周年UI_longLevel"];
        },
      },
      aloneEquip: {
        name: '<b><font color="#99FF75">单独装备栏',
        intro:
          '<b><font color="#99FF75">切换玩家装备栏为单独装备栏或非单独装备栏，初始为单独装备栏，根据个人喜好调整',
        init: true,
        update: function () {
      ...
      loadingStyle: {
        ...
        item: {
          off: "关闭",
          on:
            '<div style="width:60px;height:40px;position:relative;background-image: url(' +
            lib.assetURL +
            'extension/十周年UI/assets/image/dialog2.png);background-size: 100% 100%;"></div>',
          On:
            '<div style="width:60px;height:40px;position:relative;background-image: url(' +
            lib.assetURL +
            'extension/十周年UI/assets/image/dialog1.png);background-size: 100% 100%;"></div>',
          othersOn:
            '<div style="width:60px;height:40px;position:relative;background-image: url(' +
            lib.assetURL +
            'extension/十周年UI/assets/image/dialog3.png);background-size: 100% 100%;"></div>',
        },
        update: function () {
          if (window.decadeUI)
            ui.arena.dataset.loadingStyle =
              lib.config["extension_十周年UI_loadingStyle"];
        },
      },
      //手杀UI
      FL1: {
      ...
      jindutiaoYangshi: {
        ...
        init: "1",
        intro:
          "切换进度条样式，可根据个人喜好切换手杀进度条或十周年进度条，切换后重启生效",
        item: {
          1: "手杀进度条",
          2: "十周年PC端进度条",
          3: "十周年客户端进度条",
      ...
      jindutiaotuoguan: {
        ...
        init: false,
        intro:
          "开启进度条的情况下，开启此选项后，当玩家的进度条时间走完时，将自动托管。",
      },
      jindutiaoST: {
        name: "进度条时间间隔",
        init: "100",
