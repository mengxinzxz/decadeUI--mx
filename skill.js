...
decadeModule.import(function (lib, game, ui, get, ai, _status) {
  decadeUI.animateSkill = {
    ...
    mx_start: {
      ...
      content: function () {
        ...
        if (lib.config.extension_十周年UI_newDecadeStyle == "on") {
          ...
          var scale =
            Math.min(
              animation.canvas.width / sz.x,
              animation.canvas.height / sz.y
            ) * 0.76;
          animation.playSpine({
    ...
    mx_longLevel: {
      ...
      filter: function (event, player) {
        return (
          lib.config.extension_十周年UI_longLevel == "sex" ||
          lib.config.extension_十周年UI_longLevel == "seven"
        );
      },
      content: function () {
  ...
  decadeUI.skill = {
        ...
        event.switchToAuto = function () {
          ...
          for (var i = 0; i < results.length; i++) {
            ...
            for (var j = 0; j < results[i].length; j++) {
              ...
              if (
                player.node.judges.childNodes.length > 0 &&
                !player.hasWuxie() &&
                results[i][j] == "wuxie"
              ) {
                costs[i].value += 5;
    ...
    xinfu_zuilun: {
      ...
      prompt: function (event, player) {
        ...
        if (!player.getStat("damage")) num--;
        return (
          get.prompt("xinfu_zuilun") + "（可获得" + get.cnNumber(num) + "张牌）"
        );
      },
      content: function () {
        "step 0";
        ...
        var zuiLun = decadeUI.content.chooseGuanXing(
          player,
          cards,
          cards.length,
          null,
    ...
    xunxun: {
      ...
      content: function () {
        ...
        var player = event.player;
        var xunxun = decadeUI.content.chooseGuanXing(
          player,
          cards,
          cards.length,
          null,
        ...
        event.switchToAuto = function () {
          var cards = decadeUI.get.bestValueCards(
            xunxun.cards[0].concat(),
            player
          );
          var time = 500;
          for (var i = 0; i < 2; i++) {
    ...
    xinfu_dianhua: {
      ...
      content: function () {
        ...
        var dialog = decadeUI.content.chooseGuanXing(
          player,
          cards,
          cards.length,
          null,
        ...
        game.broadcast(
          function (player, cards) {
            if (!window.decadeUI) return;
            decadeUI.content.chooseGuanXing(
              player,
              cards,
              cards.length,
              null,
        ...
        event.switchToAuto = function () {
          ...
          if (judges.length) {
            cheats = decadeUI.get.cheatJudgeCards(
              cards,
              judges,
              friend != null
            );
    ...
    zongxuan: {
        ...
        if (_status.currentPhase == player) {
          ...
          if (get.attitude(player, next) > 0) {
            if (judges.length > 0) {
              ...
              for (var i = 0; i < cards.length; i++)
                if (get.value(cards[i], next) > 4) return true;
            }
          } else {
            if (judges.length > 0) {
              for (var j = 0; j < judges.length; j++) {
      ...
      content: function () {
        ...
        game.broadcast(
          function (player, cards, callback) {
            if (!window.decadeUI) return;
            var zongxuan = decadeUI.content.chooseGuanXing(
              player,
              cards,
              cards.length
            );
        ...
        event.switchToAuto = function () {
          ...
          while (parent != null && parent.name != "phaseDiscard")
      ...
      subSkill: {
        discard: {
          ...
          filter: function (event, player) {
            ...
            for (var i = 0; i < event.cards2.length; i++) {
              if (
                get.suit(event.cards2[i], event.player) == "club" &&
                get.position(event.cards2[i], true) == "d"
              ) {
                return true;
              }
            }
              ...
              for (var i = 0; i < cards.length; i++) {
                ...
                setTimeout(
                  function (card, index, finished) {
                    dialog.move(card, index, 1);
                    if (finished)
                      dialog.finishTime(cards.length <= 1 ? 250 : 1000);
                  },
                  time,
                  cards[i],
                  i,
  ...
  decadeUI.inheritSkill = {
        ...
        var chooseButton = function (id, name) {
          ...
          canvas.onmousedown = function (ev) {
            ...
            canvas.onmousemove = function (e) {
              ...
              ctx.beginPath();
              ctx.clearRect(
                e.offsetX / game.documentZoom - 16,
                e.offsetY / game.documentZoom - 16,
                32,
                32
              );
          ...
          canvas.ontouchstart = function (ev) {
            ...
            canvas.ontouchmove = function (e) {
              ...
              var X =
                ((e.touches[0].clientX / game.documentZoom - rect.left) /
                  rect.width) *
                canvas.width;
              var Y =
                ((e.touches[0].clientY / game.documentZoom - rect.top) /
                  rect.height) *
                canvas.height;
    ...
    dddfenye: {
      $compareFenye: function (players, cards1, targets, cards2) {
        ...
        game.addVideo("compareFenye", [
          get.targetsInfo(players),
          get.cardsInfo(cards1),
      ...
      async content(event, trigger, player) {
        ...
        if (character != player.storage.huashen.current) {
          ...
          game.log(
            player,
            "将性别变为了",
            "#y" + get.translation(lib.character[character].sex) + "性"
          );
      ...
      filter: function (event, player) {
        return (
          ...
          (event.skill.indexOf("guhuo_") == 0 ||
            event.skill.indexOf("xinfu_guhuo_") == 0)
        );
      },
      content: function () {
        "step 0";
        ...
        if (
          ...
          (card.name == "sha" &&
            (trigger.card.nature || card.nature) &&
            trigger.card.nature != card.nature)
        )
          event.fake = true;
    ...
    old_guhuo_guess: {
      ...
      content: function () {
        ...
        if (event.fake) {
          ...
          player.markAuto("old_guhuo_cheated", [
            trigger.card.name + trigger.card.nature,
          ]);
        }
        player.popup(trigger.card.name, "metal");
        player.lose(card, ui.ordering).relatedEvent = trigger;
            ...
            if (!ally && get.attitude(player, evt.player) < 0) {
              ...
              if (
                evt.player
                  ...
                  .contains(card.name + card.nature)
              )
