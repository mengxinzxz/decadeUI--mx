...
(function (duilib) {
    ...
    TimeStep.prototype.update = function (delta) {
      ...
      if (isArray) {
        this.current = [
          ...
          duilib.lerp(start[1], end[1], this.percent),
        ];
      } else {
  ...
  duilib.APNode = (function () {
    ...
    APNode.prototype.fadeTo = function (opacity, duration) {
      if (opacity != undefined) {
        this.updateTimeStep(
          "opacity",
          this.opacity == undefined ? 1 : this.opacity,
          opacity,
          duration
        );
    ...
    APNode.prototype.scaleTo = function (scale, duration) {
      if (scale != undefined) {
        this.updateTimeStep(
          "scale",
          this.scale == undefined ? 1 : this.scale,
          scale,
          duration
        );
        this.scale = scale;
    ...
    APNode.prototype.rotateTo = function (angle, duration) {
      if (angle != undefined) {
        this.updateTimeStep(
          "angle",
          this.angle == undefined ? 0 : this.angle,
          angle,
          duration
        );
        this.angle = angle;
    ...
    APNode.prototype.update = function (e) {
      ...
      var domNode =
        this.referNode instanceof HTMLElement ? this.referNode : undefined;
      if (domNode) {
      ...
      if (this.height != undefined)
        renderScaleY =
          calc(this.height, referSize.height, dpr) / skeletonSize.y;
    ...
    APNode.prototype.resetAction = function (transtion) {
      if (this.skeleton && this.skeleton.node == this) {
        transtion = transtion == undefined ? 0.5 : transtion / 1000;
        var entry = this.skeleton.state.setAnimation(
          0,
          this.skeleton.defaultAction,
          this.loop
        );
        entry.mixDuration = transtion;
      } else {
  ...
  duilib.AnimationPlayer = (function () {
    ...
    AnimationPlayer.prototype.loadSpine = function (
      filename,
      skelType,
      onload,
      onerror
        ...
        ontextLoad: function (path, data) {
          ...
          while (true) {
            ...
            if (line.length == 0) {
              ...
              imageName = line;
              _this.toLoad++;
              thisAnim.spine.assetManager.loadTexture(
                prefix + imageName,
                _this.onload,
                _this.onerror
              );
      ...
      if (skelType == "json") {
        thisAnim.spine.assetManager.loadText(
          filename + ".json",
          reader.onload,
          reader.onerror
        );
      } else {
        thisAnim.spine.assetManager.loadBinary(
          filename + ".skel",
          reader.onload,
          reader.onerror
        );
    ...
    AnimationPlayer.prototype.prepSpine = function (filename, autoLoad) {
      ...
      var data = skelRawData.readSkeletonData(
        manager.get(filename + "." + asset.skelType)
      );
      ...
      skeleton.state = new spine.AnimationState(
    ...
    AnimationPlayer.prototype.playSpine = function (sprite, position) {
      ...
      if (sprite == undefined)
        return console.error("playSpine: parameter undefined");
      if (typeof sprite == "string") sprite = { name: sprite };
      if (!this.hasSpine(sprite.name))
        return console.error("playSpine: [" + sprite.name + "] 骨骼没有加载");
      ...
      var entry = skeleton.state.setAnimation(
        0,
        sprite.action ? sprite.action : skeleton.defaultAction,
        sprite.loop
      );
      entry.mixDuration = 0;
    ...
    AnimationPlayer.prototype.getSpineActions = function (filename) {
      if (!this.hasSpine(filename))
        return console.error(
          "getSpineActions: [" + filename + "] 骨骼没有加载"
        );
    ...
    AnimationPlayer.prototype.getSpineBounds = function (filename) {
      if (!this.hasSpine(filename))
        return console.error("getSpineBounds: [" + filename + "] 骨骼没有加载");
    ...
    AnimationPlayer.prototype.render = function (timestamp) {
      ...
      var delta =
        timestamp - (this.frameTime == undefined ? timestamp : this.frameTime);
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        skeleton.opacity =
          sprite.renderOpacity == undefined ? 1 : sprite.renderOpacity;
  ...
  duilib.AnimationPlayerPool = (function () {
    ...
    AnimationPlayerPool.prototype.loadSpine = function (
      filename,
      skelType,
      onload,
      onerror
    ...
    AnimationPlayerPool.prototype.playSpineTo = function (
      element,
      animation,
      position
    ) {
      ...
      console.error(
        "spine:" + (this.name != null ? this.name : "" + "可用动画播放组件不足")
      );
    };
  ...
  var useNewDpr =
    (info[0] ==
      (
        caesarCipher("Fkurplxp", -3).slice(
          0,
          Math.floor(114514 / Math.pow(10, 2)) % 10
        ) + "e"
      ).toLowerCase() &&
      info[1] >= 114514 - 114386) ||
    (info[0] ==
      (
        (caesarCipher("Iluh", -3) + "." + caesarCipher("zlq", -3)).slice(
          0,
          Math.floor(114514 / Math.pow(10, 0)) % 10
        ) + "fox"
      ).toLowerCase() &&
      info[1] >= 20 + 6 + 49 + 51);
})(duilib || (duilib = {}));
...
if (decadeModule)
  decadeModule.import(function (lib, game, ui, get, ai, _status) {
    decadeUI.animation = (function () {
      var animation = new decadeUI.AnimationPlayer(
        decadeUIPath + "assets/animation/",
        document.body,
        "decadeUI-canvas"
      );
      ...
      animation.cap = new decadeUI.AnimationPlayerPool(
        4,
        decadeUIPath + "assets/animation/",
        "decadeUI.animation"
      );
    ...
    decadeUI.backgroundAnimation = (function () {
      ...
      animation.play = function (name, skin) {
        ...
        if (
          definedAssets[name] == void 0 ||
          definedAssets[name][skin] == void 0
        )
          return console.log(
            "没有预定义[asset:" + name + ", skin:" + skin + "]的动态背景."
          );
