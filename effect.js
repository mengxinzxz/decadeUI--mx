...
decadeModule.import(function (lib, game, ui, get, ai, _status) {
  decadeUI.effect = {
      ...
      compare: function (source, target) {
        ...
        (dialog.set = function (attr, value) {
          switch (attr) {
            ...
            case "card1":
            case "sourceCard":
              if (dialog.cards[0].firstChild)
                dialog.cards[0].removeChild(dialog.cards[0].firstChild);
              dialog.cards[0].appendChild(value);
              break;

            case "card2":
            case "targetCard":
              if (dialog.cards[1].firstChild)
                dialog.cards[1].removeChild(dialog.cards[1].firstChild);
    ...
    kill: function (source, target) {
      if (get.itemtype(source) != "player" || get.itemtype(target) != "player")
        throw "arguments";
      if (source == target) return;

      if (source.isUnseen() || target.isUnseen()) return;

      var sourceAvatar = source.isUnseen(0)
        ? source.node.avatar2
        : source.node.avatar;
      var targetAvatar = target.isUnseen(0)
        ? target.node.avatar2
        : target.node.avatar;
      ...
      killerWarpper.killer.style.backgroundImage =
        sourceAvatar.style.backgroundImage;
      ...
      victim.back.part1.style.backgroundImage =
        targetAvatar.style.backgroundImage;
      victim.back.part2.style.backgroundImage =
        targetAvatar.style.backgroundImage;
    ...
    skill: function (player, skillName, vice) {
      ...
      image.onload = function () {
        bgImage.onload = function () {
          ...
          sprite.scale = Math.max(
            animation.canvas.width / size.x,
            animation.canvas.height / size.y
          );
        ...
        bgImage.src =
          decadeUIPath + "assets/image/bg_xianding_" + camp + ".png";
      };
      ...
      image.onerror = function () {
        ...
        image.src =
          lib.assetURL +
          "imagearacter/default_silhouette_" +
